...
- Data and persistence stance: no database; skills are stateless. Model content and rationale live inside the Archi model via MCP documentation fields. The repo holds skills, helpers, tests, and evidence.
- Quality bar: each objective is verified by structural checks (skills reference only real MCP tools) plus one documented live end-to-end scenario per specialist against a real Archi model via the MCP, with evidence under `docs/evidence/`.
