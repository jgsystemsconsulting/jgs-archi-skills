...
- **Never modify** the jgs-archi-mcp plugin, its handlers, or its shipped resources (VISION NG-1).
- **Never copy** ArchiMate language reference tables into skills. Read `archimate://reference/*` and `archimate://recipes/*` from the MCP (VISION NG-4).
