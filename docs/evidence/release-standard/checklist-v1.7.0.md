...
| 10 | Landing page RR-B-20 | PASS | `docs/index.html` + `docs/.nojekyll`. Pages from `master:/docs`. Latest build `built` (commit 9fbddf1). Live URL https://jgsystemsconsulting.github.io/jgs-archi-skills/ returns HTTP 200. Repo made public 2026-08-29 so Pages could enable. |
...
| 14 | Branch protection RR-B-23 | PASS | Applied after the repo went public: PR + `integrity` check, `allow_force_pushes=false`, `allow_deletions=false`, `enforce_admins=false`, 0 required approvals (solo-maintainer shape). |
