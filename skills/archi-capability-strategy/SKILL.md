...
Orchestrator-dispatched after View Plan confirmation when the run needs capability and strategy modelling.
...
Model the **strategy** concern set: Capabilities, resources, courses of action, value streams, and strategic alignment to motivation outcomes.
...
| Layer scope | yes | This specialist owns **strategy** (and explicit adjacencies only when hand-off says so) |
...
1. Restate layer scope and target viewpoints (Capability map / strategy viewpoint).
...
4. Prefer types appropriate to the **strategy** layer per MCP reference resources.
...
```markdown
## Specialist Result: archi-capability-strategy
