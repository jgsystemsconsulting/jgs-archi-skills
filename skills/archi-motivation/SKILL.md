...
Orchestrator-dispatched after View Plan confirmation when the run needs motivation-layer modelling.
...
Model the **motivation** concern set: Drivers, goals, principles, requirements, stakeholders, assessments, and outcomes that justify change.
...
| Layer scope | yes | This specialist owns **motivation** (and explicit adjacencies only when hand-off says so) |
...
1. Restate layer scope and target viewpoints (Motivation viewpoint / stakeholder concerns view).
...
4. Prefer types appropriate to the **motivation** layer per MCP reference resources.
...
```markdown
## Specialist Result: archi-motivation
