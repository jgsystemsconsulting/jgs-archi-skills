...
FROZEN = "3fa7f235d168d6037b1448f9ce440e7c67a37ca120f17ab95b934b2593e56534"
...
class SpecialistContractTests(unittest.TestCase):
    ...
    def test_create_path_house_style_helpers(self) -> None:
        text = CREATE.read_text(encoding="utf-8")
        self.assertIn("docs/MODELLING_CONVENTIONS.md", text)
        self.assertIn("folder_convention.py", text)
        self.assertIn("--require-evidence", text)
        self.assertIn("aspect-hints", text)
        self.assertIn("get-folders", text)
        self.assertIn("move-to-folder", text)

    def test_model_qa_house_style_helpers(self) -> None:
        text = (ROOT / "skills" / "archi-model-qa" / "SKILL.md").read_text(
            encoding="utf-8"
        )
        self.assertIn("docs_coverage.py", text)
        self.assertIn("--require-evidence", text)
        self.assertIn("aspect-hints", text)
        self.assertIn("folder_convention.py", text)
        self.assertIn("get-folders", text)
